...
class FEA_T(nn.Module):
    ...
    def forward(self, x, state=None):
        ...
        # preprossess
        x_in = x_in.squeeze(1).to(x.device)  # x: (B, 1, 1024, 2) -> (B, 1024, 2)
        B = x_in.shape[0]
...
class FEAT(nn.Module):
    def __init__(self, patch_size=32,  # Frame length L (default value: 32)
                 d_fix_qk=16,  # The embedding dimension of queries and keys of each head d_p (default value: 16)
                 d_fix_v=16,  # The embedding dimension of values of each head d_p (default value: 16)
                 seq_length=1024,  # The length of the input signal sequence L (default value: 16)
                 hidden_features=64 * 4,  # The dimension of inner-layer of FFN (default value: 4L for DB-GLU)
        ...
        super().__init__()
        self.encoder = FEA_T(patch_size=patch_size,
                             d_fix_qk=d_fix_qk, d_fix_v=d_fix_v,
                             seq_length=seq_length, hidden_features=hidden_features,
                             n_head_qk=n_head_qk, n_head_v=n_head_v,
                             overlap=overlap, dropout=dropout, layer_num=layer_num,
                             num_class=num_class, pos_emb=pos_emb, bias=bias,
                             talking=talking, real_former=real_former,
                             ffn_type=ffn_type, activation=activation, feature_mode=feature_mode)
        in_features = patch_size * 2
        self.classifier = nn.Linear(in_features, num_class)
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 1, 2, 128).to(device)
    Net = FEAT(seq_length=128, num_class=11).to(device)
    print(Net(x).shape)