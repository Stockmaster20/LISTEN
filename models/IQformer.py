...
class IQFormer(nn.Module):
    ...
    def forward(self, x, stft):
        ...
        x = self.norm(x)
        fea = self.globalavgpool(x)
        cls_out = self.head(fea)
        return cls_out
    
if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = IQFormer([2,2,2], embed_dims=[64,64,64],
        mlp_ratios=4,
        act_layer=nn.GELU,
        num_classes=11,
        ...
        fork_feat=False,).to(device=device)
    input_tensor = torch.randn(1, 2, 128).to(device=device)  # batch=1
    stft_tensor = torch.randn(1, 1, 32, 128).to(device=device)  # batch=1
    output = model(input_tensor, stft_tensor)
    print(output.shape)